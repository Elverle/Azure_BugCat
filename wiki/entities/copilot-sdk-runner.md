...
- When FT-14C enables MCP, builds a session-scoped `mcpServers` object with the resolved Azure DevOps MCP binary, organization slug, encoded PAT env, and wildcard tool exposure.
...
- Copilot does not rely on `.mcp.json`; FT-14C keeps its MCP configuration entirely in-memory and attaches it only when the constructed object passes a basic shape check.
- `supportsFixMode` remains `false`, while `supportsMcp` is now `true`.
