...
| `agent:start`                   | ✅ Implemented | Resolves the current bug, selected project(s), and settings, computes FT-14C MCP availability, starts a bounded FT-14E session, persists the running snapshot, caches the session MCP status for summaries, and returns `{ sessionId, agentProvider, mcpStatus }` |
