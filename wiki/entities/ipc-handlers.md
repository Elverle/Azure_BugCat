...
| `llm:categorize`        | ✅ Implemented | Loads settings+session, creates a per-window `AbortController`, calls `categorizeBugs()`, sends progressive `ChunkProgress`, persists only on success |
