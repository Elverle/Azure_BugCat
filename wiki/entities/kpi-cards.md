...
- Falls back to `No assignees` when no non-null assignees exist in the current slice. `computeKpis()` skips null assignees outright, so the `UNASSIGNED` sentinel never reaches this card.
