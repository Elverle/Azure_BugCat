...
- Applies pending migrations in ascending version order and persists `schemaVersion` after each successful step.
- Writes migrated `settings` and `session` back to the store only after the pipeline completes.
